...
fn can_load_regular_contract() -> Result<()> {
    ...
    // then
    assert_eq!(contract.code(), std::fs::read(contract_binary)?);
...
fn can_manually_create_regular_contract() -> Result<()> {
    // given
    let contract_binary = "sway/contracts/contract_test/out/release/contract_test.bin";
    let binary = std::fs::read(contract_binary)?;

    // when
    let contract = Contract::regular(binary.clone(), Salt::zeroed(), vec![]);
...
fn regular_contract_has_expected_getters() -> Result<()> {
    let contract_binary = "sway/contracts/contract_test/out/release/contract_test.bin";
    let contract = Contract::load_from(contract_binary, LoadConfiguration::default())?;
    let expected_binary = std::fs::read(contract_binary)?;
    let expected_contract_id =
        "d62e0d1326dd2aca749657c59c3288c0bdd0fe664482f72d3a85a9e289ff92cb".parse()?;
    let expected_state_root =
        "8aaf36bbaf5ef509ba6013e11b0f7176de429c9b6b3c1b09cc50b36e3113b901".parse()?;
    let expected_code_root =
        "d6721f6198f3765615222390655ce1acfe2ce1ec06002527cfa786b6d705ad0b".parse()?;
...
fn regular_can_be_turned_into_loader_and_back() -> Result<()> {
    ...
    let contract_original = Contract::load_from(contract_binary, LoadConfiguration::default())?;

    let loader_contract = contract_original.clone().convert_to_loader(1024)?;
...
fn unuploaded_loader_contract_has_expected_getters() -> Result<()> {
    ...
    let original = Contract::load_from(contract_binary, LoadConfiguration::default())?;
...
fn uploaded_loader_has_expected_getters() -> Result<()> {
    let contract_binary = "sway/contracts/huge_contract/out/release/huge_contract.bin";
    let original_contract = Contract::load_from(contract_binary, LoadConfiguration::default())?;
