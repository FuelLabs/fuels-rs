...
The storage slots of a contract can be initialized automatically from the JSON file generated by the sway compiler.
...
We can initialize the slots both automatically and manually by using `StorageConfiguration::new()`.
If we this, then the generated `Vec<StorageSlot>` will be merged and the `manually` generated `Vec<StorageSlot>` takes precedence.
