                ...
                match token {
                    #fuels_types_path::Token::Struct(tokens) => {
                        ...
                        let mut next_token = move || { tokens_iter
                            .next()
                            .ok_or_else(|| { #fuels_types_path::errors::Error::Other(#std_lib::format!("ran out of tokens before `{}` has finished construction", #struct_name_str)) })
                        };
                    ...
                    other => ::core::result::Result::Err(#fuels_types_path::errors::Error::Other(#std_lib::format!("error while constructing `{}`. Expected token of type `Token::Struct`, got `{:?}`", #struct_name_str, other))),
                }
            }
        }
    })
        ...
        impl #impl_gen #fuels_core_path::traits::Tokenizable for #name #type_gen #where_clause {
                ...
                Self: Sized,
            {
                match token {
                    ...
                    _ => ::core::result::Result::Err(#std_lib::format!("token `{}` is not of the type `Token::Enum`", token)),
                }.map_err(|e| #fuels_types_path::errors::Error::Other(#std_lib::format!("error while instantiating `{}` from token `{}`", #name_stringified, e)) )
            }
        }
    })
}
