...
async fn get_single_wallet() -> LocalWallet {
    let single_wallet_config = WalletsConfig::new(Some(1), None, None);
    let mut wallets = launch_provider_and_get_wallets(single_wallet_config).await;
...
async fn test_provider_launch_and_connect() {
    ...
    let mut wallet = LocalWallet::new(None);
