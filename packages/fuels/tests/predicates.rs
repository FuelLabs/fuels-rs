...
#[tokio::test]
async fn spend_predicate_coins_messages_raw_slices() -> Result<()> {
    abigen!(Predicate(
        name = "MyPredicateEncoder",
        abi = "packages/fuels/tests/predicates/predicate_raw_slice/out/debug/predicate_raw_slice-abi.json"
    ));
    ...
    let mut predicate: Predicate = Predicate::load_from(
        "tests/predicates/predicate_raw_slice/out/debug/predicate_raw_slice.bin",
    )?
    .with_data(predicate_data);
