...
pub mod abigen_bindings {
    ...
    pub mod my_script_test_mod {
        ...
        impl<T: ::fuels::signers::Account + ::fuels::signers::PayFee + ::std::clone::Clone> MyScriptTest<T>
        where
            ::fuels::types::errors::Error: From<<T as ::fuels::signers::PayFee>::Error>,
        {
            ...
            pub fn main(
                &self,
                bim: self::Bimbam,
                bam: self::SugarySnack,
            ) -> ::fuels::programs::script_calls::ScriptCallHandler<T, self::Bimbam> {
                let script_binary = ::std::fs::read(&self.binary_filepath)
                    .expect("Could not read from binary filepath");
                let encoded_args = ::fuels::core::abi_encoder::ABIEncoder::encode(&[
                    ::fuels::types::traits::Tokenizable::into_token(bim),
                    ::fuels::types::traits::Tokenizable::into_token(bam),
                ])
